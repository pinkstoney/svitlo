...
size_t ShutdownData::m_writeCallback(void *contents, size_t size, size_t nmemb,
                                     void *userp) {
...
void ShutdownData::setPostData(const std::string &choice,
  ...
  if (choice == "accountNumber") {
    ...
    throw std::invalid_argument(
        "Invalid choice. Please choose either 'accountNumber' or 'address'.");
  }
}
...
void ShutdownData::processHour(const nlohmann::json &hourData, bool isToday) {
  ...
  auto &willBeElectricity =
      isToday ? m_willBeElectricityToday : m_willBeElectricityTomorrow;
  auto &mightBeElectricity =
      isToday ? m_mightBeElectricityToday : m_mightBeElectricityTomorrow;
  auto &wontBeElectricity =
      isToday ? m_wontBeElectricityToday : m_wontBeElectricityTomorrow;
...
void ShutdownData::formatElectricityData(const std::string &rawData) {
  ...
  try {
    ...
    throw std::runtime_error(
        "Invalid JSON data. Please check the correctness of your input.");
  }

  if (!data.contains("graphs"))
    throw std::runtime_error(
        "The data does not contain information for today/tomorrow.");
...
ShutdownData::getWillBeElectricityToday() const {
  return m_willBeElectricityToday;
}

const std::vector<std::pair<int, int>> &
ShutdownData::getMightBeElectricityToday() const {
  return m_mightBeElectricityToday;
}

const std::vector<std::pair<int, int>> &
ShutdownData::getWontBeElectricityToday() const {
  return m_wontBeElectricityToday;
}

const std::vector<std::pair<int, int>> &
ShutdownData::getWillBeElectricityTomorrow() const {
  return m_willBeElectricityTomorrow;
}

const std::vector<std::pair<int, int>> &
ShutdownData::getMightBeElectricityTomorrow() const {
  return m_mightBeElectricityTomorrow;
}

const std::vector<std::pair<int, int>> &
ShutdownData::getWontBeElectricityTomorrow() const {
...
void ShutdownData::setQueue(int queue) { m_queue = queue; }

void ShutdownData::setSubqueue(int subqueue) { m_subqueue = subqueue; }

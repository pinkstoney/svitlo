...
void Application::m_displayInputScreen()
{
    if (m_isInternetConnected)
    {
        ...
        if (IsKeyPressed(KEY_ENTER) && m_isAddressEntered && m_errorMessage.empty())
        {
...
void Application::processData(const std::string &inputInfo)
{
    ...
    try
    {
        std::string userChoice = std::all_of(inputInfo.begin(), inputInfo.end(), ::isdigit) ? "accountNumber"
                                                                                            : "address";

...
public:
    ...
    void createTables()
    {
        ...
        CREATE TABLE IF NOT EXISTS schedule_approved (
            consumer_id TEXT,
            date TEXT,
            approved_since TEXT,
            FOREIGN KEY(consumer_id) REFERENCES consumers(id)
    ...
    {
        const char* sql1 = "INSERT OR REPLACE INTO consumers (id, queue, subqueue) VALUES (?, ?, ?);";
        ...
        const char* sql2 = "INSERT OR REPLACE INTO electricity_data (consumer_id, date, hour, status) VALUES (?, ?, ?, ?);";
        auto stmt2 = prepareStatement(sql2);

        const char* sql3 = "INSERT OR REPLACE INTO schedule_approved (consumer_id, date, approved_since) VALUES (?, ?, ?);";
        auto stmt3 = prepareStatement(sql3);
        ...
        for (const auto& date : {"today", "tomorrow"})
        {
            for (int hour = 0; hour < 24; ++hour)
            {
                ...
                bindText(stmt2.get(), 2, date);
                bindInt(stmt2.get(), 3, hour);
                bindInt(stmt2.get(), 4, static_cast<int>(consumer.getElectricityStatus(date, hour)));

                executeStatement(stmt2.get());
                sqlite3_reset(stmt2.get());
            }

            std::string approvedSince = consumer.getScheduleApprovedSince(date);
            if (!approvedSince.empty())
            {
                bindText(stmt3.get(), 1, consumer.getId());
                bindText(stmt3.get(), 2, date);
                bindText(stmt3.get(), 3, approvedSince);

                executeStatement(stmt3.get());
                sqlite3_reset(stmt3.get());
            }
        }
    }
    ...
    std::unique_ptr<Consumer> loadConsumer(const std::string& id)
    {
        ...
        sql = "SELECT date, hour, status FROM electricity_data WHERE consumer_id = ?;";
        stmt = prepareStatement(sql);
        bindText(stmt.get(), 1, id);

        while (::sqlite3_step(stmt.get()) == SQLITE_ROW)
        {
            std::string date = reinterpret_cast<const char*>(::sqlite3_column_text(stmt.get(), 0));
            int hour = ::sqlite3_column_int(stmt.get(), 1);
            ElectricityData::Status status = static_cast<ElectricityData::Status>(::sqlite3_column_int(stmt.get(), 2));
            consumer->setElectricityStatus(date, hour, status);
        }

        sql = "SELECT date, approved_since FROM schedule_approved WHERE consumer_id = ?;";
        stmt = prepareStatement(sql);
        bindText(stmt.get(), 1, id);
        ...
        {
            std::string date = reinterpret_cast<const char*>(::sqlite3_column_text(stmt.get(), 0));
            std::string approvedSince = reinterpret_cast<const char*>(::sqlite3_column_text(stmt.get(), 1));
            consumer->setScheduleApprovedSince(date, approvedSince);
        }
